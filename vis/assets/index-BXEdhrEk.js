...
`).replace(Xy, "");
  ...
  var kb = zb();
  const Mb = "" + new URL("gossip_sim_wasm_bg-Bt8RWnX7.wasm", import.meta.url).href, Lb = async (l = {}, a) => {
    let o;
...
`);
  ...
  function w1() {
    const l = Mo();
    if (!l) return null;
    const { graph: a, highlightedNode: o, setHighlightedNode: i, events: u, time: c, layout: d } = l;
    if (!u) return null;
    const h = ae.useMemo(() => ({
