...
async fn accept_loop(topics: TopicMap, conn: Connection, max_message_size: usize) {
    loop {
        ...
        let Some(topic) = topics.get(&topic_id) else {
            debug!(topic=%topic_id.fmt_short(), "dropping stream for a topic we have not joined");
            continue;
        };
        let msg = TopicMessage::RemoteStream {
...
impl TopicHandle {
    fn new(topic_id: TopicId, shared: Arc<Shared>, topics: WeakTopicMap) -> (Self, TopicActor) {
...
struct TopicActor {
    ...
    remote_senders: HashMap<EndpointId, SendQueue>,
...
impl TopicActor {
    ...
    async fn handle_connected(
        &mut self,
        (remote, sender): (EndpointId, n0_error::Result<Guarded<GossipSender>>),
    ) {
        match sender {
            Ok(sender) => {
        ...
        let sender = match self.remote_senders.entry(remote) {
            ...
            hash_map::Entry::Vacant(entry) => {
                ...
                self.connecting.push(Box::pin(async move {
                    (remote, connect(&shared, remote, topic).await)
                }));
