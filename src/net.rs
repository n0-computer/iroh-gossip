...
struct Actor {
    ...
    rpc_rx: mpsc::Receiver<self::rpc::Message>,
...
impl Actor {
    ...
    async fn handle_rpc_msg(&mut self, msg: rpc::Message, now: Instant) -> Result<(), Error> {
        use rpc::Message::*;
...
mod test {
    ...
    async fn gossip_net_big() -> TestResult {
        ...
        for i in 0..bootstrap_count {
            ...
            let (sender, mut receiver) = gossips[i].subscribe(topic_id, bootstrap).await?.split();
        ...
        for chunk in 1..chunks {
            ...
            for j in 0..bootstrap_count {
                ...
                let (sender, mut receiver) =
                    gossips[i].subscribe(topic_id, bootstrap).await?.split();
                let endpoint = routers[i].endpoint().clone();
                senders.push((sender, endpoint.node_id()));
                joining.push(
                    async move {
